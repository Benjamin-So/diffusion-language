...
class BCJaxPyPolicy(py_policy.PyPolicy):
  ...
  def _run_action_inference(self, observation):
    # Add a batch dimension to each element in the observation dictionary.
    observation = jax.tree_util.tree_map(lambda x: jnp.expand_dims(x, 0), observation)
    # Apply the model to get the normalized action for the observation.
    normalized_action = self.model.apply(
        self.variables, observation, train=False)
    # Denormalize the action using action standard deviation and mean.
    action = (
        normalized_action * jnp.maximum(self.action_std, EPS) +
        self.action_mean)

    # Clip the action to be within the specified minimum and maximum range.
  ...
  def _action(self, time_step, policy_state=(), seed=0):
    ...
    # wraps the action into a PolicyStep named tuple, ensuring that the action is returned alongside the (empty) policy state and (empty) info
